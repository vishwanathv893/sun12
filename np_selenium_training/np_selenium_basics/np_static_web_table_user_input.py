...
for j in range(2,8):#each row is accessed
    book_name =driver.find_element(By.XPATH,f"//table[@name='BookTable']/tbody/tr{[j]}/td{[1]}")#locating the elements in a table using xpath with string formating
    books_on_table.append(book_name.text)#storing the book names in the list DS using append functions    
...
while i<=5:
    if entered_book_name == books_on_table[i] :
        price_of_book =driver.find_element(By.XPATH,f"//table[@name='BookTable']/tbody/tr{[i+2]}/td[4]")#findind the price value on static web table
